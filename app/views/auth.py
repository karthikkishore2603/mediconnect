...
def login_verify():
    ...
    jwt_token = util.create_access_token(
        schemas.TokenData(
            username=username,
            id=crud.get_hospital_id_by_username(username),
            user_type="hospital",
        ),
...
def patient_login_post():
    ...
    patient_id = crud.get_patient_id(patient_phoneno)
    if patient_id:
        response = make_response(redirect(url_for("patient_dashboard")))
        jwt_token = util.create_access_token(
            schemas.TokenData(
                username=patient_phoneno,
                id=patient_id,
                user_type="patient",
            ),
            timedelta(days=constants.ACCESS_TOKEN_EXPIRE_DAYS),
        )
        response.set_cookie(
            constants.AUTH_TOKEN_COOKIE_NAME,
            jwt_token,
